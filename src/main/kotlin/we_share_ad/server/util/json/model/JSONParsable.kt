...
/**
 * [JSONObject]에 대응하기 위한 wrapper class
 *
 * @author 김지환
 */
                ...
                if (key == formatted || toUnderscore(key) == formatted) {
                    ...
                    when (field.type) {
                        ...
                        JSONList::class.java -> {
                            ...
                            val genericClass = informedClass.asSubclass(JSONParsable::class.java)
                            field.set(this, JSONList(jsonObject.getJSONArray(key), genericClass))
                        }
                        AdCategory.First::class.java -> {
                            field.set(this, AdCategory.getFirstAdCategory(jsonObject.getInt(key)))
                        }
                        AdCategory.Second::class.java -> {
                            val categoryFirst = jsonObject.getInt("category_first")
                            field.set(this, AdCategory.getSecondAdCategory(categoryFirst, jsonObject.getInt(key)))
                        }
                        AdSize::class.java -> {
                            field.set(this, AdSize.findSize(jsonObject.getInt(key)))
                        }
                        List::class.java -> {
                            // List 무시
                        }
